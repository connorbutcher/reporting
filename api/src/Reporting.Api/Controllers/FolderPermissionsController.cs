...
{
    [HttpGet]
    public async Task<ActionResult<PermissionsDto>> Get(Guid folderId)
    {
    ...
    [HttpGet("audit")]
    public async Task<ActionResult<List<GrantAuditEntryDto>>> Audit(Guid folderId)
    {
    ...
    [HttpPut("inheritance")]
    public async Task<IActionResult> SetInheritance(Guid folderId, SetInheritanceDto dto) =>
        await admin.SetFolderInheritanceAsync(folderId, dto) ? NoContent() : NotFound();

    [HttpPut]
    public async Task<ActionResult<AccessGrantDto>> Upsert(Guid folderId, SaveGrantDto dto)
    {
        try
        {
    ...
    [HttpDelete]
    public async Task<IActionResult> Remove(Guid folderId, [FromBody] RemoveGrantDto dto)
    {
        try
        {
