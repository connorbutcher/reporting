...
public class FolderRepository(ReportingDbContext db)
{
    ...
    public async Task<List<FolderDto>> GetChildrenAsync(Guid? parentId)
    {
    ...
    public async Task<FolderDto> CreateAsync(string name, Guid? parentFolderId)
    {
        if (parentFolderId is { } parentId && !await db.Folders.AnyAsync(f => f.Id == parentId))
        {
    ...
    /// </summary>
    public async Task<FolderDto?> UpdateAsync(Guid id, string name, Guid? parentFolderId)
    {
        ...
        if (parentFolderId is { } parentId)
        {
            ...
            var cursor = await db.Folders.FirstOrDefaultAsync(f => f.Id == parentId);
            if (cursor is null) throw new DataValidationException("Parent folder does not exist.");

            var visited = new HashSet<Guid>();
            while (cursor is not null)
            {
                if (cursor.Id == id) throw new DataValidationException("Cannot move a folder into its own subtree.");
    ...
    public async Task<bool> DeleteAsync(Guid id)
    {
        ...
        var hasChildFolders = await db.Folders.AnyAsync(f => f.ParentFolderId == id);
        var hasReports = await db.Reports.AnyAsync(r => r.FolderId == id);
